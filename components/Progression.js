...
export default function Progression({ user }) {
  ...
  const handleClaimDailyReward = async () => {
    if (!user || rewardHasClaimedToday || claimingDailyReward) return
    ...
    try {
      ...
      const currentRewardStreak = currentData?.streak || 0
      const currentCoins = currentData?.coins || 0
      const continuedStreak = lastClaimedFromDB ? isDateYesterday(lastClaimedFromDB) : false
      const newRewardStreak = !lastClaimedFromDB
        ? 1
        : continuedStreak
          ? currentRewardStreak + 1
          : 1
      const rewardIndex = Math.min(newRewardStreak - 1, STREAK_REWARDS.length - 1)
      const rewardAmount = STREAK_REWARDS[rewardIndex]
      const updatedCoins = currentCoins + rewardAmount

      const { error: updateError } = await supabase
        .from('user_pass')
        .update({
          last_claimed: today,
          streak: newRewardStreak,
          coins: updatedCoins,
      ...
      setShopFeedback({
        type: 'success',
        msg: `+${rewardAmount} CocoCoins ! Série : ${newRewardStreak} jour${newRewardStreak > 1 ? 's' : ''}`
      })
      setTimeout(() => setShopFeedback(null), 3000)
    } catch (error) {
      console.error('[DailyReward] Error while claiming reward:', error)
  ...
  const quizCardMeta = useMemo(() => {
    const attempts = quizHistoryStats?.attempts || []
    const paddedAttempts = [
      ...Array(Math.max(0, 7 - attempts.length)).fill(null),
      ...attempts
    ]
    const lastAttempt = attempts[attempts.length - 1]
    const status = quizDoneToday ? (quizState.success ? 'success' : 'failed') : 'pending'
    const lastAttemptLabel = lastAttempt?.date
      ? new Date(lastAttempt.date).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'short' })
      : null
  ...
  }, [quizHistoryStats, quizDoneToday, quizState?.success])

  const { paddedAttempts: paddedQuizAttempts, status: quizStatus, lastAttemptLabel: lastQuizAttemptLabel } = quizCardMeta

  const rewardTodayKey = new Date().toISOString().slice(0, 10)
  const rewardHasClaimedToday = stats.lastClaimed === rewardTodayKey
  const rewardStreakLength = stats.streak || 0
  const rewardCycleSteps = STREAK_REWARDS.length
  const rewardCyclePosition = rewardStreakLength % rewardCycleSteps
  const rewardFilledSteps = rewardStreakLength === 0
    ? 0
    : (rewardCyclePosition === 0 ? rewardCycleSteps : rewardCyclePosition)
  const rewardNextStreakValue = !stats.lastClaimed
    ? 1
    : isDateYesterday(stats.lastClaimed)
      ? rewardStreakLength + 1
      : 1
  const rewardUpcomingStep = rewardHasClaimedToday
    ? (rewardFilledSteps === rewardCycleSteps ? 1 : rewardFilledSteps + 1)
    : rewardNextStreakValue
  const rewardNextClaimAmount = STREAK_REWARDS[Math.min(rewardNextStreakValue, STREAK_REWARDS.length) - 1]
  const rewardUpcomingAmount = STREAK_REWARDS[Math.min(rewardUpcomingStep, STREAK_REWARDS.length) - 1]
  const rewardCycleDisplayDay = rewardHasClaimedToday
    ? (rewardFilledSteps === 0 ? 1 : rewardFilledSteps)
    : rewardNextStreakValue
  const rewardClaimedStepsCount = rewardHasClaimedToday
    ? rewardFilledSteps
    : Math.max(rewardFilledSteps, rewardNextStreakValue - 1)
  const rewardStepStates = Array.from({ length: rewardCycleSteps }).map((_, idx) => {
    const stepNumber = idx + 1
    const isClaimed = rewardClaimedStepsCount >= stepNumber && rewardClaimedStepsCount > 0
    const isNext = rewardUpcomingStep === stepNumber
    return {
      stepNumber,
      amount: STREAK_REWARDS[idx],
      isClaimed,
      isNext
    }
  })
  const rewardSummaryText = rewardHasClaimedToday
    ? 'Récompense quotidienne récupérée. La série continue !'
    : `Réclame ta récompense pour atteindre ${rewardNextStreakValue} jour${rewardNextStreakValue > 1 ? 's' : ''} consécutif${rewardNextStreakValue > 1 ? 's' : ''}.`
  const rewardHelperText = rewardHasClaimedToday
    ? `Reviens demain pour débloquer +${rewardUpcomingAmount} CocoCoins.`
    : `Cette étape te rapporte +${rewardNextClaimAmount} CocoCoins.`
  const rewardLastClaimLabel = stats.lastClaimed
    ? new Date(stats.lastClaimed).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })
    : null
  const rewardButtonLabel = rewardHasClaimedToday
    ? '✅ Déjà récupéré aujourd\'hui'
    : `🎁 +${rewardNextClaimAmount} CocoCoins`
  const rewardButtonText = claimingDailyReward ? 'Récupération...' : rewardButtonLabel
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 600, color: '#b45309', marginBottom: 4 }}>
                    ...
                    <div style={{
                      ...
                      <span>🪙</span>
                      +{rewardUpcomingAmount}
                    </div>
                    <div style={{ fontSize: '0.7rem', color: '#92400e', fontWeight: 600 }}>
                      Jour {Math.min(rewardCycleDisplayDay, rewardCycleSteps)} / {rewardCycleSteps}
                    </div>
                  </div>
                </div>
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: `repeat(${rewardCycleSteps}, 1fr)`,
                  gap: 6,
                  marginBottom: 12
                }}>
                  {rewardStepStates.map(step => (
                    <div key={step.stepNumber} style={{ textAlign: 'center' }}>
                      <div
                        style={{
                          height: 36,
                          borderRadius: 12,
                          background: step.isClaimed
                            ? 'linear-gradient(135deg,#f97316,#fb923c)'
                            : step.isNext
                              ? 'rgba(234, 179, 8, 0.35)'
                              : 'rgba(255, 255, 255, 0.65)',
                          color: step.isClaimed ? 'white' : '#b45309',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          fontWeight: 700,
                          border: step.isNext ? '2px solid rgba(234, 179, 8, 0.6)' : '1px solid rgba(250, 204, 21, 0.35)',
                          boxShadow: step.isClaimed ? '0 4px 12px rgba(249, 115, 22, 0.25)' : 'none'
                        }}
                        title={`Jour ${step.stepNumber} · +${step.amount} CocoCoins`}
                      >
                        {step.isClaimed ? '✔︎' : step.stepNumber}
                      </div>
                      <div style={{ fontSize: '0.65rem', fontWeight: 600, color: '#92400e', marginTop: 4 }}>
                        +{step.amount}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ fontSize: '0.75rem', color: '#b45309', fontWeight: 600, marginBottom: rewardLastClaimLabel ? 8 : 12 }}>
                  {rewardHelperText}
                </div>
                {rewardLastClaimLabel && (
                  <div style={{ fontSize: '0.7rem', color: '#b45309', fontWeight: 600, marginBottom: 12 }}>
                    Dernière récupération : {rewardLastClaimLabel}
                  </div>
                )}
                <button
                  onClick={handleClaimDailyReward}
                  disabled={rewardHasClaimedToday || claimingDailyReward}
                  style={{
                    background: rewardHasClaimedToday
                      ? 'rgba(253, 224, 71, 0.45)'
                      : 'linear-gradient(135deg,#f59e0b,#fbbf24)',
                    color: rewardHasClaimedToday ? '#92400e' : 'white',
                    border: 'none',
                    borderRadius: 12,
                    padding: '10px 18px',
                    fontWeight: 800,
                    fontSize: '0.95rem',
                    cursor: rewardHasClaimedToday ? 'not-allowed' : 'pointer',
                    width: '100%',
                    boxShadow: rewardHasClaimedToday ? 'none' : '0 6px 18px rgba(234, 179, 8, 0.35)',
                    transition: 'all 0.2s ease',
                    opacity: rewardHasClaimedToday ? 0.8 : 1
                  }}
                >
                  {rewardButtonText}
                </button>
                <div style={{ fontSize: '0.7rem', color: '#b45309', fontWeight: 600, marginTop: 8 }}>
                  Manquer une journée réinitialise la série.
                </div>
              </div>
              <div style={{

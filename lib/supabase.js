...
export async function uploadImage(file, fileName) {
  try {
    logInfo(`Upload d'image: ${fileName}`, {
      fileSize: file.size,
      fileType: file.type,
      fileName: fileName
    });
    ...
    logDebug('Configuration d\'upload', {
      fileName,
      contentType,
      fileSize: file.size,
    ...
    if (error) {
      logError('Erreur lors de l\'upload d\'image', error, {
        fileName,
        fileSize: file.size,
        contentType,
    ...
    logError('Erreur critique lors de l\'upload d\'image', error, {
      fileName,
      fileSize: file?.size,
...
export async function uploadImageWithRetry(file, fileName, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      logDebug(`Tentative d'upload ${attempt}/${maxRetries}`, { fileName });
      ...
      logInfo(`Upload réussi à la tentative ${attempt}`, { 
      ...
      logWarning(`Tentative ${attempt} échouée`, error, { 
        fileName, 
        attempt, 
        maxRetries,
  ...
  logError(`Échec de l'upload après ${maxRetries} tentatives`, lastError, { fileName });
  throw lastError;
}
...
export async function createImageStorageBucket() {
  try {
    ...
    if (bucketExists) {
      logInfo('Bucket recipe-images existe déjà');
      return true;
    }
    ...
    if (error) {
      ...
      console.log(`
=== CRÉATION MANUELLE DU BUCKET REQUISE ===

Veuillez créer manuellement le bucket 'recipe-images' dans votre dashboard Supabase :
...
=== FIN DES INSTRUCTIONS ===
    ...
    logInfo('Bucket recipe-images créé avec succès');
    return true;
  } catch (error) {
    logError('Erreur critique lors de la création du bucket', error);
    return false;
  }
}

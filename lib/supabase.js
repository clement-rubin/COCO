...
export async function createImageStorageBucket() {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      logError(`[${bucketId}] Configuration Supabase incomplète`, new Error('Missing env vars'), {
        hasUrl: !!supabaseUrl,
        hasKey: !!supabaseAnonKey
      });
      return false;
...
Problème détecté: Clé API invalide ou expirée

Solutions:
1. Vérifiez vos variables d'environnement dans .env.local
2. Copiez la nouvelle clé depuis votre dashboard Supabase:
   - Allez dans Settings > API
   - Copiez la "anon public" key
   - Mettez à jour NEXT_PUBLIC_SUPABASE_ANON_KEY
...
3. Redémarrez votre serveur de développement (npm run dev)
    ...
    if (recipeImagesBucket) {
      ...
      if (!recipeImagesBucket.public) {
        logError(`[${bucketId}] Bucket non public`, new Error('Bucket not public'), {
          bucketId,
          bucketPublic: recipeImagesBucket.public
        });
        
        console.log(`
❌ BUCKET NON PUBLIC DÉTECTÉ

Le bucket 'recipe-images' existe mais n'est pas configuré comme public.

Solutions:
1. Dans votre dashboard Supabase:
   - Allez dans Storage > Buckets
   - Cliquez sur 'recipe-images'
   - Cliquez sur 'Settings' (⚙️)
   - Cochez "Public bucket"
   - Cliquez sur "Save"

2. Ou supprimez et recréez le bucket en suivant les instructions du README
        `);
        return false;
      }
      
      // Test 3: Vérifier les permissions d'accès
      try {
        ...
        if (listFilesError) {
          logWarning(`[${bucketId}] Problème d'accès au bucket`, listFilesError, {
            ...
            statusCode: listFilesError.statusCode
          });
...
Le bucket existe et est public, mais il y a un problème de permissions.

Solutions:
1. Exécutez les politiques SQL depuis le README dans votre SQL Editor
2. Ou recréez le bucket en suivant toutes les étapes du README
3. Vérifiez que votre clé API est correcte
          ...
          logInfo(`[${bucketId}] Bucket recipe-images configuré correctement`, {
            ...
            isPublic: recipeImagesBucket.public
          });
        }
    ...
    // Test 4: Le bucket n'existe pas, afficher les instructions
    logInfo(`[${bucketId}] Bucket recipe-images non trouvé`);
    
    console.log(`
❌ BUCKET MANQUANT

Le bucket 'recipe-images' n'existe pas dans votre projet Supabase.
...
4. Nom: recipe-images
5. ⚠️ IMPORTANT: Cochez "Public bucket"
6. Cliquez sur "Create bucket"
7. Exécutez les politiques SQL du README
8. Revenez sur cette page et actualisez

Ne tentez pas de création automatique - cela pourrait échouer.
    `);
    
    return false;
    
  } catch (error) {
    logError(`[${bucketId}] Erreur critique lors de la vérification du bucket`, error, {
      bucketId,
      errorName: error.name,
      errorMessage: error.message,
      errorStack: error.stack
...
Une erreur inattendue s'est produite lors de la vérification du bucket.

Vérifications à faire:
1. Votre connexion internet fonctionne
2. Vos variables d'environnement sont correctes
3. Votre projet Supabase est actif

...
// Nouvelle fonction pour valider et convertir les images de recettes - UTILISE LES UTILITAIRES
...
// Fonction utilitaire pour convertir des bytes en image URL - VERSION RENFORCÉE
export function bytesToImageUrl(bytesArray) {
  ...
  if (!Array.isArray(bytesArray)) {
    logDebug('bytesToImageUrl: bytesArray n\'est pas un array', { 
      type: typeof bytesArray, 
      isString: typeof bytesArray === 'string',
      value: String(bytesArray).substring(0, 50)
    })
  ...
  // Vérifier que tous les éléments sont des nombres valides
  const invalidBytes = bytesArray.filter(byte => typeof byte !== 'number' || byte < 0 || byte > 255)
  if (invalidBytes.length > 0) {
    logError('bytesToImageUrl: Bytes invalides détectés', new Error('Invalid byte values'), {
      bytesLength: bytesArray.length,
      invalidBytesCount: invalidBytes.length,
      sampleInvalidBytes: invalidBytes.slice(0, 5)
    })
    return null
  }
  ...
  try {
    ...
    let detectedFormat = 'jpeg' // par défaut
    for (const [format, header] of Object.entries(imageHeaders)) {
      if (header.every((byte, index) => uint8Array[index] === byte)) {
        detectedFormat = format
        break
      }
    }
    
    logDebug('Format d\'image détecté', { detectedFormat, firstBytes: Array.from(uint8Array.slice(0, 8)) })
    
    // Conversion en base64 avec chunks pour éviter les erreurs de taille
    let base64 = ''
    ...
    const mimeType = detectedFormat === 'jpeg' ? 'image/jpeg' : 

  ...
  try {
    ...
    if (friendships && friendships.length > 0) {
      ...
      const profileByUserId = new Map(
        (friendProfiles || []).map(profile => [profile.user_id, profile])
      )
      ...
      for (const friendship of friendships) {
        ...
        if (!otherUserId || uniqueFriends.has(otherUserId)) {
          continue
        }

        uniqueFriends.set(otherUserId, {
          ...friendship,
          friend_id: otherUserId,
          friend_profile: profileByUserId.get(otherUserId) || {
            user_id: otherUserId,
    ...
    if (pendingFriendships && pendingFriendships.length > 0) {
      const requesterIds = Array.from(new Set(pendingFriendships.map(f => f.user_id).filter(Boolean)))
      ...
      const requesterByUserId = new Map(
        (requesterProfiles || []).map(profile => [profile.user_id, profile])
      )
      const uniquePendingRequests = new Map()

      for (const friendship of pendingFriendships) {
        if (!friendship.user_id || uniquePendingRequests.has(friendship.user_id)) {
          continue
        }

        uniquePendingRequests.set(friendship.user_id, {
          ...friendship,
          requester_profile: requesterByUserId.get(friendship.user_id) || {
      ...
      pendingRequests = Array.from(uniquePendingRequests.values())
    }
    
    return {
      friends: friends || [],
      pendingRequests: pendingRequests || [],
      error: (friends.length === 0 && pendingRequests.length === 0) ? (friendsError || pendingError) : null
    }
  } catch (error) {

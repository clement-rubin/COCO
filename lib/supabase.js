  ...
  try {
    ...
    if (!checkError) {
      ...
      if (columnError && columnError.code === 'PGRST204') {
        ...
        console.log(`
=== MISE À JOUR DU SCHÉMA POUR LES URLs ===

Votre table doit être mise à jour pour utiliser des URLs au lieu de bytea :

-- Modifier le type de la colonne image
ALTER TABLE public.recipes 
ALTER COLUMN image TYPE text;

-- Ou si vous créez une nouvelle table :
CREATE TABLE public.recipes (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  title text NOT NULL,
  description text,
  image text, -- Changé de bytea à text pour stocker les URLs
  prepTime text,
  cookTime text,
  category text,
  author text,
...
=== FIN DU SQL ===
    ...
    // Si la table n'existe pas, afficher les instructions avec le bon schéma
    if (checkError && (checkError.code === 'PGRST106' || checkError.code === '42P01')) {
      logInfo('Table recipes non trouvée, création manuelle requise');
      
      console.log(`
=== CRÉATION AVEC SUPPORT URLs ===

CREATE TABLE public.recipes (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  title text NOT NULL,
  description text,
  image text, -- text pour stocker les Data URLs ou URLs
  prepTime text,
  cookTime text,
  category text,
  author text,

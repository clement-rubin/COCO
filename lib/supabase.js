...
=== FIN DES INSTRUCTIONS ===
    ...
    if (error) {
      logError(`[${bucketId}] Erreur lors de la création du bucket`, error, {
        bucketId,
...
=== FIN DES INSTRUCTIONS ===
    ...
    logInfo(`[${bucketId}] Bucket recipe-images créé avec succès`, {
      bucketId,
      bucketData: data
    });
    return true;
    ...
    logError(`[${bucketId}] Erreur critique lors de la création du bucket`, error, {
      bucketId,
      errorName: error.name,
      errorMessage: error.message
    });

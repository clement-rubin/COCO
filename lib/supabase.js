...
export async function createRecipesTableIfNotExists() {
  ...
  try {
    ...
    if (!checkError) {
      ...
      // Vérifier si toutes les colonnes nécessaires existent selon le schéma exact
...
Votre schéma actuel semble correct. Si vous avez des erreurs, vérifiez que toutes les colonnes existent :

-- Schéma exact attendu (correspond à votre table actuelle)
...
Votre table recipes doit être créée avec exactement ce schéma :
...
// Fonction utilitaire pour convertir des bytes en image URL - VERSION AMÉLIORÉE
export function bytesToImageUrl(bytesArray) {
  ...
  if (!Array.isArray(bytesArray)) {
    logDebug('bytesToImageUrl: bytesArray n\'est pas un array', { type: typeof bytesArray, isString: typeof bytesArray === 'string' })
    
    // Si c'est déjà une chaîne base64 ou URL
    if (typeof bytesArray === 'string') {
      if (bytesArray.startsWith('data:image/') || bytesArray.startsWith('http')) {
        return bytesArray
      }
      // Supposer que c'est du base64 brut
      return `data:image/jpeg;base64,${bytesArray}`
    }
    return null
  }
  
  if (bytesArray.length === 0) {
    logDebug('bytesToImageUrl: bytesArray est vide')
    return null
  }
  
  try {
    logDebug('Conversion bytes vers URL', { 
      bytesLength: bytesArray.length,
      firstBytes: bytesArray.slice(0, 10),
      isValidRange: bytesArray.every(byte => byte >= 0 && byte <= 255)
    })
    ...
    logInfo('Image URL créée avec succès', {
      originalBytesLength: bytesArray.length,
    ...
    logError('Erreur lors de la conversion bytes vers image URL', error, {
      bytesLength: bytesArray?.length,
      bytesType: typeof bytesArray,
...
// Nouvelle fonction pour valider et convertir les images de recettes
export function getRecipeImageUrl(imageData, fallbackUrl = '/placeholder-recipe.jpg') {
  if (!imageData) {
    logDebug('getRecipeImageUrl: Aucune donnée image fournie, utilisation du fallback')
    return fallbackUrl
  }
  ...
  try {
    ...
    if (typeof imageData === 'string' && (imageData.startsWith('http') || imageData.startsWith('data:image/'))) {
      logDebug('getRecipeImageUrl: URL existante détectée')
      return imageData
    }
    
    // Si c'est une chaîne base64 brute
    if (typeof imageData === 'string' && imageData.length > 100) {
      logDebug('getRecipeImageUrl: Chaîne base64 détectée')
      return `data:image/jpeg;base64,${imageData}`
    }
    
    // Si c'est un tableau de bytes (format bytea de PostgreSQL)
    if (Array.isArray(imageData)) {
      logDebug('getRecipeImageUrl: Tableau de bytes détecté', { length: imageData.length })
      return bytesToImageUrl(imageData) || fallbackUrl
    }
    
    // Si c'est un objet avec une propriété bytes
    if (imageData && typeof imageData === 'object' && imageData.bytes) {
      logDebug('getRecipeImageUrl: Objet avec propriété bytes détecté')
      return bytesToImageUrl(imageData.bytes) || fallbackUrl
    }
    
    logWarning('getRecipeImageUrl: Format d\'image non reconnu', {
      type: typeof imageData,
      isArray: Array.isArray(imageData),
      hasBytes: !!(imageData && imageData.bytes),
      length: imageData?.length
    })
    
    return fallbackUrl
  } catch (error) {
    logError('getRecipeImageUrl: Erreur lors de la conversion', error, {

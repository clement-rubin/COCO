...
export function getRecipeImageUrl(imageData, fallbackUrl = '/placeholder-recipe.jpg') {
  try {
    // Use the improved processImageData function from imageUtils
    const { processImageData } = require('../utils/imageUtils');
    const processedUrl = processImageData(imageData, fallbackUrl);
    
    logDebug('Recipe image URL processed', {
      hasImageData: !!imageData,
      imageDataType: typeof imageData,
      isArray: Array.isArray(imageData),
      processedUrl: processedUrl?.substring(0, 50) + (processedUrl?.length > 50 ? '...' : ''),
      isFallback: processedUrl === fallbackUrl
    });

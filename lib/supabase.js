...
// Fonction pour créer la table recipes avec le nouveau schéma
export async function createRecipesTableIfNotExists() {
  logInfo('Vérification/création de la table recipes avec schéma bytea');
  
  try {
    // Vérifier si la table existe avec le bon schéma
    const { data: tableCheck, error: checkError } = await supabase
      .from('recipes')
      .select('id, title, description, image, ingredients, instructions')
      .limit(1);
    
    if (!checkError) {
      logInfo('Table recipes existe avec le schéma correct');
      return true;
    }
...
// Nouvelle fonction pour initialiser avec le schéma bytea
export async function initializeRecipesTable() {
  logInfo('Début de l\'initialisation de la table recettes (schéma bytea)');
  ...
  try {
    ...
    // Si pas de recettes, ajouter des exemples sans images pour le moment
    ...
    if (!existingRecipes || existingRecipes.length === 0) {
      ...
      const initialRecipes = [
        ...
        },
        {
          ...
          ingredients: ["12 feuilles de lasagnes", "2 courgettes", "2 aubergines", "400g de tomates pelées", "250g de ricotta", "200g de mozzarella", "100g de parmesan", "2 gousses d'ail", "Basilic frais", "Huile d'olive"],
          instructions: [
...
// Supprimer les fonctions Supabase Storage qui ne sont plus nécessaires
// export async function uploadImage() - SUPPRIMÉE
// export async function uploadImageWithRetry() - SUPPRIMÉE  
// export function getImageUrl() - SUPPRIMÉE
// export async function deleteImage() - SUPPRIMÉE
// export async function createImageStorageBucket() - SUPPRIMÉE

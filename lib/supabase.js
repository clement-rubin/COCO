...
export async function initializeFriendsSystem() {
  try {
    ...
    if (profilesError) {
      logError('Erreur création table profiles', profilesError)
      return false
    }
    
    // Create friendships table if it doesn't exist  
    const { error: friendshipsError } = await supabase.rpc('create_friendships_table_if_not_exists')
    if (friendshipsError) {
      logError('Erreur création table friendships', friendshipsError)
      return false
    }
...
export async function createUserProfileIfNotExists(user) {
  ...
  try {
    logInfo('Vérification/création profil utilisateur', {
    ...
    if (checkError && checkError.code !== 'PGRST116') {
      logError('Erreur vérification profil existant', checkError)
      return false
    }
    
    if (existingProfile) {
      logInfo('Profil utilisateur existe déjà')
      return true
    }
    ...
    if (insertError) {
      logError('Erreur création profil', insertError)
      return false
    }
    
    logInfo('Profil utilisateur créé avec succès')
    return true
    
  } catch (error) {
    logError('Erreur lors de la création du profil utilisateur', error)
    return false
  }
}

...
import { processImageData, processImageToUrl } from '../utils/imageUtils';
...
export async function uploadImageAsUrl(file) {
  ...
  try {
    ...
    // Utiliser les utilitaires d'images importées
...
// Fonction de rétrocompatibilité pour uploadImageAsBytes (ancien nom)
export async function uploadImageAsBytes(file) {
  logWarning('uploadImageAsBytes est déprécié, utilisez uploadImageAsUrl', {
    fileName: file.name,
    fileSize: file.size
  });
  
  // Rediriger vers la nouvelle fonction
  return await uploadImageAsUrl(file);
}

...
export async function createRecipesTableIfNotExists() {
  ...
  try {
    ...
    if (!checkError) {
      ...
      // Vérifier si toutes les colonnes nécessaires existent, y compris difficulty
      ...
      if (columnError && columnError.code === 'PGRST204') {
        ...
        // Instructions spécifiques pour ajouter les colonnes manquantes
        console.log(`
=== COLONNES MANQUANTES DÉTECTÉES ===

Exécutez ce SQL dans votre dashboard Supabase pour ajouter les colonnes manquantes :
...
ALTER TABLE recipes ADD COLUMN IF NOT EXISTS prepTime TEXT;
ALTER TABLE recipes ADD COLUMN IF NOT EXISTS cookTime TEXT;
ALTER TABLE recipes ADD COLUMN IF NOT EXISTS photos JSONB DEFAULT '[]'::jsonb;
ALTER TABLE recipes ADD COLUMN IF NOT EXISTS user_id UUID; -- Ajout de la colonne user_id
...
CREATE INDEX IF NOT EXISTS idx_recipes_difficulty ON recipes(difficulty);
CREATE INDEX IF NOT EXISTS idx_recipes_user_id ON recipes(user_id); -- Index pour filtrage par utilisateur
